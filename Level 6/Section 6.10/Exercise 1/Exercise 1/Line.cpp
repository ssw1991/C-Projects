...
double Line::Length(std::shared_ptr<DistanceStrategy> strat) const
{
	return m_p1.Distance(m_p2, strat);
}


void Line::Accept(std::shared_ptr<ShapeVisitor> v)
{
	v->Visit(*this);
}
...
{
	return std::shared_ptr<Shape>(new Line(*this));
};
