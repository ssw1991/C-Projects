...
shared_ptr<DistanceStrategy> Point::m_strat = shared_ptr<DistanceStrategy>();
...
void Point::SetStrat(shared_ptr<DistanceStrategy> strat)
{
	m_strat = strat;
}
...
{// Distance from self to Point p
	return m_strat->Distance(*this, p);
}

double Point::Distance() const
{//Distance from self to origin
	return m_strat->Distance(*this, *op.instance());
}
