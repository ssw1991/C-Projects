...
int main()
{
	...
	signalB.connect(1,signalC); // By calling there functions prior to the associated signal it prints in order B C D
	signalC.connect(1,signalD);
	signalB.connect(0,slotB);
	signalC.connect(0,slotC);
	signalD.connect(0,slotD1);
